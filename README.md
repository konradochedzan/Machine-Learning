...
$$
L(\beta) = -\sum_i [y_i \log(q_\beta(x_i)) + (1-y_i)\log(1-q_\beta(x_i))].
$$
...
The notebook computes the counts

$$
N_{TP}(c) = |\{i : y_i = 1,\ \hat{p}_i > c\}|,
\qquad
N_{FP}(c) = |\{i : y_i = 0,\ \hat{p}_i > c\}|,
$$
...
$$
Balance_j = R_j(B)\cdot loan\cdot interest - F_j(B)\cdot loan.
$$
...
$$
\Delta \log S_j = -\frac{1}{2}\sigma^2\Delta t + \sigma\sqrt{\Delta t}\,Z_j,\qquad Z_j \sim N(0,1).
$$
...
$$
L(H) = \hat{E}[(G - p - \sum_j H_j\Delta S_j)^2].
$$
